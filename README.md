...
It helps determine whether an action that appears operationally correct was actually grounded in a valid chain of authorization, intent, and responsibility. CML is designed for settings where knowing what happened is not enough; we also need to know why it was allowed to happen.

The core idea is central: a system may be functionally correct while being causally invalid. An action may succeed, a response may look reasonable, and a workflow may complete, but the underlying chain can still be causally unauthorized, missing parent approval, hiding a causal gap, or failing to preserve responsibility across steps.
...
CML was running as a read-only validator over the event trace. For this query it showed no causal link through the required B edge, while tracing a clean path through the incorrect C edge. In other words, the textual reasoning trace looked fine, but the recorded causal chain had drifted away from the authorized ground-truth path.
...
CML checks whether a sensitive action or state transition was causally authorized, not only whether it occurred.
...
It is not a full safety stack on its own. It is one control primitive that checks whether recorded action lineage is causally valid, responsibility-preserving, and grounded.
...
CML is best understood as infrastructure for high-risk action validation and causal legitimacy checks.
...
This repository does not claim to solve all AI safety problems. It contributes a specific control primitive: checking whether actions were causally grounded in valid authorization, intent, and responsibility chains.
...
- semantics for validating causal legitimacy and lineage coherence
...
CML does not execute code or enforce runtime policy by itself. It validates whether recorded action chains are causally authorized and grounded.
...
| Tracing (OpenTelemetry) | Execution order, latency, performance | Tracing explains execution flow. CML checks causal legitimacy of action lineage. |
...
| Transport (HTTP, TCP) | Moving data | CML does not focus on delivery; it focuses on grounded state transitions and responsibility preservation. |
...
That makes CML a strong supporting artifact for safety evaluation work focused on grounded state transitions, authorization lineage, and accountability-preserving action chains.
