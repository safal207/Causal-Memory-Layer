#!/usr/bin/env python3
"""Protected entrypoint for CML Retrieval v0.1."""
...
import memory_retrieval_github as github  # noqa: E402
...
def read_event(path: Path) -> dict[str, Any]:
    try:
        ...
        raise core.RetrievalError(f"cannot read GitHub event payload: {exc}") from exc
...
def main() -> None:
    ...
    try:
        ...
        if args.pull_number is not None:
            pull_number = _positive_int(args.pull_number, label="pull number")
        else:
        ...
        result = github.retrieve_for_pull(
    ...
    if not result.get("passed", False):
        raise SystemExit(f"CML retrieval failed closed: {result.get('outcome')}")
    print(
        f"CML retrieval outcome={result['outcome']} "
        f"pull={result.get('pull_number')} selected={len(result.get('selected', []))} "
