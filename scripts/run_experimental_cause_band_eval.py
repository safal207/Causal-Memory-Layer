...
def _parse_duration_threshold(raw: Any, default: int = 3) -> int:
...
def _max_consecutive_outside_safe(ranks: list[int]) -> int:
    current = 0
    best = 0
    for rank in ranks:
...
def evaluate_fixture(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    duration_threshold = _parse_duration_threshold(policy.get("duration_threshold"), default=3)

    findings: list[str] = []
    if any(rank >= BAND_RANK["warning_range"] for rank in ranks):
        findings.append(RANGE_DRIFT)

    max_consecutive = _max_consecutive_outside_safe(ranks)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Experimental fixture path (default: {DEFAULT_FIXTURE})",
    )
    parser.add_argument(
        "--json",
        action="store_true",
